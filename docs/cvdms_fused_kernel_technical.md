...
> **注意**：`max_terms` 和 `max_inner` 是两个独立参数，分别控制外层 Taylor 和内层 K-series 的级数展开，互不替代。原 `order` 参数（控制 BSC 1/k 修正截断）已移除，1/k 修正现使用逐像素收敛循环（与 CGS `calOneDevideK_forward_back` 等效）。
...
BSC 参数（共用）: `convergence_threshold=1e-7`
...
3. **1/k 修正级数** (`cvdms.py:797-838`):
   公式使用 `(1 + K/(πK0))^(-1/2)` 的二项式展开，使用逐像素收敛循环
   （`n_above = sum(|scratch| > threshold)` + 停滞检测）而非固定截断阶数。
   30 keV 下 `K/(πK0)` 较大。增大 `max_inner` 对 I/I₀ 无改善（见下方 order 测试表），
   说明二项式展开本身已收敛。
...
**1/k 修正收敛阶数测试**（2026-05-09，通过 `max_inner` 控制上限）：

| max_inner | max I/I₀ excess | BSC Σ\|BSC\|²/I₀ | 结论 |
|-------|-----------------|-------------------|------|
| 1（注） | +0.5747% | 7.56×10⁻⁵ | 基准 — 一阶截断 (此行为旧 `order=1`) |
| 2 | +0.5687% | 7.52×10⁻⁵ | **无改善** (差异 ~0.006%) |
| 3 | +0.5694% | 7.54×10⁻⁵ | **无改善** (差异 ~0.005%) |

> **结论**：增大 1/k 修正的收敛上限对 I/I₀ > 1 问题**无改善**。该级数在首项后已收敛。
...
**采样率 + Laplacian 方法对比**（2026-05-09, st=0.4 Å）：
...
30 keV 下 CVDMS+BSC 的 I/I₀ > 1 是**算法近似而非数值 bug**，涉及 SBA 框架、
前向传播器与 BSC 之间的误差耦合、以及 K 级数差分运算的数值条件数三个层面。

#### 11.4.1 SBA 框架的非幺正性

单背散射近似（SBA）的核心假设是背散射波远弱于前向波，因此采用微扰处理：

```
ψ_total = ψ_forward + ε·ψ_backward + O(ε²)
```

SBA 在此做了一次截断：忽略 ψ_backward 对 ψ_forward 的反馈（即多次背散射）。
这个截断**破坏了幺正性**——完整的多片层散射是幺正的（概率守恒），但 SBA
等价于从耦合波方程中删除了反向耦合项，使得概率不再严格守恒。

**I/I₀ > 1 的直接原因**：在 SBA 框架下，背散射修正项 `B(ψ)` 是从前向波中
减去一个"背散射损失"，但这个减法的幅度和相位由近似算子决定，不保证
`|ψ - B(ψ)| ≤ |ψ|`。当背散射项的振幅过估计或相位偏差超过 90° 时，
强度反而增加。

#### 11.4.2 前向传播器误差与 BSC 误差的耦合

这是最关键的间接机制。前向 CVDMS 传播器本身有截断误差 ε_T：

```
ψ_forward = ψ_exact + ε_T · ψ_err + O(ε_T²)
```

其中 ε_T 来自泰勒级数有限项截断（`conv_thresh=1e-6` 控制，但不为零）。

BSC 算子作用在前向波上产生差异：

```
B(ψ_forward) = B(ψ_exact + ε_T·ψ_err)
             = B(ψ_exact) + ε_T·B(ψ_err) + O(ε_T²)
```

误差项 ε_T·B(ψ_err) 在以下条件下不可忽略：
1. BSC 算子放大高频成分（拉普拉斯项 ∇²/(4πK₀) 在精细采样下放大）
2. 前向误差 ψ_err 恰好落在 BSC 高响应的频率区间
3. 修正后 exit_wave = ψ_forward - B(ψ_forward)，误差项叠加而非抵消

在 30 keV + 0.03 Å 条件下，三点同时成立：

- K₀ = 14.3 Å⁻¹ → k_critical = 2.17 Å⁻¹，BSC 放大倍率 = O(k²/K₀²) ≈ 10
- 前向泰勒截断误差集中在高频（拉普拉斯算子的收敛慢于势能项）
- B(ψ_err) 与前向波 ψ_exact 的相位关系使得 `ψ_forward - B(ψ_forward)`
  的模长 > |ψ_forward|，出现非物理增益

#### 11.4.3 K 级数差分运算的条件数灾难

BSC 的核心操作是两个相邻片层 K 级数的差分：

```
B(ψ) = (K_j - K_{j-1}) / (2·K_j) · ψ
```

在 ∂z → 0 的极限下，K_j ≈ K_{j-1}，差分变为两个接近相等的数值相减。
设 S = K_j 在数值上的值，ΔS = K_j - K_{j-1}，则差分的条件数：

```
κ ≈ ‖S‖ / ‖ΔS‖ × |forward_error_amplification|
```

对 SrTiO₃ 30 keV、采样 0.03 Å：
...
| 条件数 κ | ~2000 |
| float32 精度 | ~1.2 × 10⁻⁷ |
| 有效精度损失 | ~log₁₀(2000) ≈ 3.3 位十进制 |

**结果**：float32 的 7 位有效数字中损失约 3 位，仅剩 ~4 位可靠。
这解释了为什么改用 complex128（15 位有效数字）后 I/I₀ 趋于 1。

#### 11.4.4 电压依赖性与采样耦合

I/I₀ > 1 问题并非某一电压所独有——所有电压下均可出现，取决于采样精细程度。
问题由电压和采样共同决定：

**电压效应**：BSC 算子幅值与 K₀ 成反比，低电压的 BSC 修正更大。

**采样效应**：条件数 κ ∝ 1/|ΔS|，而 |ΔS| 随采样变细而减小（相邻片层差异变小）。
精细采样同时提高 Nyquist 频率，允许更多高频成分进入 K 级数，进一步增大 κ。

因此，不同电压只是"触发阈值"不同：

| 电压 | K₀ (Å⁻¹) | B 相对幅值 | 临界采样（出现 I/I₀ > 1） |
|------|----------|-----------|------------------------|
| 300 keV | 50.79 | 1.0× (基准) | ≤ 0.03 Å 时出现 |
| 80 keV | 24.93 | 2.0× | ≤ 0.05 Å 时出现 |
| 30 keV | 14.30 | 3.5× | ≤ 0.10 Å 时出现 |
| 10 keV | 8.17 | 6.2× | ≤ 0.15 Å 时出现 |

300 keV 在常规采样（≥ 0.1 Å）下无问题，是因为 BSC 修正幅值小，条件数 κ 在
float32 容忍范围内。但当采样加密到 0.03 Å 时，相邻片层差异 ΔS 减小，
条件数 κ 增大到同样触发 I/I₀ > 1。这是一个**量变而非质变**——所有电压在足够
精细的采样下都会出现强度不守恒，仅是触发采样阈值不同。

#### 11.4.5 强度单调递减失败的机制

物理上弹性散射的透射束强度应随厚度单调递减（量子力学概率流守恒）。
实测 CVDMS+BSC 在 30 keV + 细采样下 I/I₀ 呈波动，根因如下：

1. **BSC 修正过度**：在特定片层，背散射项 B(ψ) 与前向波 ψ 反相位叠加，
   导致 `|ψ - B(ψ)| > |ψ|`
2. **误差随片层积累**：前向 CVDMS 的泰勒截断误差在每片层有系统性方向（非随机），
   跨片层积累后与 BSC 修正产生共振
3. **SBA 反馈缺失**：真实物理中背散射损失的能量应在前向波中体现为衰减，
   但 SBA 的单向近似使得修正项在特定条件下"反向馈入"前向通道

#### 11.4.6 缓解策略（按优先级）

| 优先级 | 策略 | 机理 | cost |
|--------|------|------|------|
| 1 | 后处理重归一化 | 每片层 `exit_wave *= sqrt(I₀/I)` | 零 |
| 2 | complex128 | 条件数从 2000 降至有效精度损失可忽略 | 2× 显存, 1.5× 时间 |
| 3 | 反相位 BSC 截断 | 当 `|ψ - B(ψ)| > |ψ|` 时跳过当前片层的 BSC | 零 |
| 4 | 物理约束 BSC | BSC 修正强制为纯衰减：`ψ *= exp(-|B(ψ)|/|ψ|)` | 低 |
| 5 | 粗采样回退（≥ 0.1 Å） | 降低 Nyquist 频率 → 减小 BSC 条件数 | 精度损失 |

**推荐**：优先级 1（后处理重归一化）应在 CVDMSMultislice 中默认启用；
优先级 3（反相位截断）作为额外安全网。两项改动均不改变现有数值结果，
仅在检测到非物理增益时触发修正。<!-- 后续版本实施 -->
