...
\textbf{空间结构。}投影势$V(\mathbf{R})=\sum_a V_a(\mathbf{R}-\mathbf{R}_a)$在原子柱位置达到峰值，$V_\text{peak}\propto Z^{4/3}$，$1/e$宽度$w_\text{col}\sim\SIrange{0.4}{0.7}{\angstrom}$，在间隙区域下降1--3个数量级~\cite{LobatoVanDyck2014}。采用Lobato参数化~\cite{LobatoVanDyck2014}和各向同性Debye--Waller因子（在\SI{300}{\kelvin}下，Sr: 0.62, Ti: 0.51, O: 0.86, Si: 0.46, Au: 0.66~\SI{}{\angstrom\squared}~\cite{Forbes2010}），代表性峰值为：SrTiO$_3$~[001] Sr柱$V_\text{peak}\approx\SI{1060}{\electronvolt\angstrom}$；Si~[001]柱$\approx\SI{470}{\electronvolt\angstrom}$；Au~[001]柱$\approx\SI{2300}{\electronvolt\angstrom}$。
...
通过三组解析极限确立了CVDMS前向传播子的基线正确性。对自由空间传播（$V=0$），CVDMS在$t=\SI{400}{\angstrom}$范围内精确重现解析Fresnel传播子$\exp(i\Delta z\,\nabla_\perp^2/(4\pi K_0))$：在128$\times$128（\SI{0.20}{\angstrom\per\pixel}）和627$\times$627（\SI{0.05}{\angstrom\per\pixel}）两种网格上，通量在机器精度内守恒，$|1-\sum|\psi|^2|$在float32舍入误差内保持为零（图\ref{fig:vacuum}）。在弱相位极限$\sigma V_\text{proj}\Delta z\ll1$下，传播子恢复程函近似$\psi\approx\exp(i\sigma V_\text{proj})$，相位RMS误差低于$10^{-5}$~rad；而常势垒$V_0$返回解析平面波解，在627$\times$627网格、$t=\SI{400}{\angstrom}$条件下振幅RMS误差为$3.2\times10^{-6}$（图\ref{fig:srtio3_unitarity}）。

转向材料态传播，以SrTiO$_3$ [001]在\SI{30}{\keV}、$\Delta z=\SI{0.4}{\angstrom}$为基准检验幺正性。无背散射修正时，通量守恒满足$|\sum|\psi|^2-1|<10^{-6}$。启用Fresnel BSC算符将幺正残差$|T|^2+|R|^2-1$降至$10^{-7}$以下，在SrTiO$_3$/真空和SrTiO$_3$/Si两种界面均成立（图\ref{fig:bsc_residual}）；相比之下，简化背散射近似（SBA）在相同条件下产生高达$+17\%$的非物理通量增益，作为有记录的负对照。轴向概率流$j_z(\mathbf{R},z)=\Im[\psi^*\,\partial_z\psi]/(2\pi K_0)$提供了更严格的检验，满足$\partial_z\langle j_z\rangle/\langle j_z\rangle<10^{-6}$每片——它同时探测相位和振幅，不同于单纯的$\sum|\psi|^2$守恒（图\ref{fig:bsc_stability}）。
...
\caption{\textbf{真空幺正性。}自由空间传播（$V=0$）下，CVDMS精确重现解析Fresnel传播子。左右面板分别显示Stage~A（\SI{0.20}{\angstrom\per\pixel}）和Stage~B（\SI{0.05}{\angstrom\per\pixel}）；通量守恒$I/I_0=1$在两个网格上均在机器精度内成立。}\label{fig:vacuum}
\end{figure}
...
\caption{\textbf{Fresnel BSC幺正性。}(a) SrTiO$_3$ [001]在\SI{30}{\keV}下前向$I/I_0$随深度变化。(b) $\sum|\text{BSC}|^2$随深度变化。(c) 能量收支：SrTiO$_3$/真空和SrTiO$_3$/Si界面处$|T|^2+|R|^2-1<10^{-7}$，而SBA产生$+17\%$通量误差。}\label{fig:bsc_residual}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/v2_divergence.pdf}
\caption{\textbf{BSC在扩展厚度下的稳定性。}(a) $I/I_0$和(b) $|1-I/I_0|$随深度变化，SrTiO$_3$ [001]在\SI{30}{\keV}下，跨越100个原胞（$\approx$\SI{390}{\angstrom}，976片），启用BSC修正和带限。\SI{390}{\angstrom}处$\sim$1.04\%的通量损失与每原子层$\sim10^{-4}$反射概率的累积一致。}\label{fig:bsc_stability}
\end{figure}
...
切片厚度细化在SrTiO$_3$ [001]在\SI{30}{\keV}下进行，保持总厚度$t=\SI{400}{\angstrom}$不变，改变$\Delta z\in\{0.4,0.2,0.1,0.05\}$~\AA{}（对应$N_z\in\{1000,2000,4000,8000\}$片）。相邻细化水平之间的归一化互相关单调趋近$1-10^{-5}$，离散化误差按$\mathcal O(\Delta z^2)$标度，与二阶Taylor截断一致（图\ref{fig:slice_refinement}）。

收敛阈值细化通过改变$\varepsilon\in[10^{-5},10^{-9}]$进行测试。外Taylor阶数按$n_\text{outer}\sim\log_{10}(1/\varepsilon)+3$对数增长，而内K迭代在$\varepsilon\le10^{-7}$时饱和。可观测物理量——CBED盘强度和出射波相位——在$\varepsilon\le10^{-7}$时稳定到相对误差$<10^{-4}$（图\ref{fig:eps_convergence}）。
...
\caption{\textbf{切片厚度细化。}SrTiO$_3$ [001]在\SI{30}{\keV}下，保持总厚度$t=\SI{400}{\angstrom}$不变，改变$\Delta z$。(a) 相邻细化水平间及与最细$\Delta z$参考的NCC。(b) $1-$NCC随$\Delta z$变化（双对数），附$\mathcal O(\Delta z^2)$参考线，确认二阶Taylor截断误差标度。}\label{fig:slice_refinement}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/v4_convergence_A.pdf}
\caption{\textbf{收敛阈值细化。}SrTiO$_3$ [001]在\SI{30}{\keV}下改变$\varepsilon\in[10^{-5},10^{-9}]$。(a) 相对于$\varepsilon=10^{-9}$参考的$\Delta I/I_0$随$\varepsilon$变化。(b) $1-$NCC随$\varepsilon$变化。可观测量在$\varepsilon\le10^{-7}$时稳定。}\label{fig:eps_convergence}
\end{figure}
...
势场与诊断量之间的空间关联是定量的。$V$与$n^*$之间的Pearson相关系数在Sr/Ti柱上超过0.9，原子柱周围的径向剖面$\langle n^*(R)\rangle$以5\%精度追踪$V_\text{col}(R)$。Fresnel BSC反射率$|R(\mathbf{R})|^2$与$|\partial_z V|^2$相关，如一阶连续性展开所预测（第\ref{sec:bsc}节）。诊断量进一步分辨原子种类：在SrTiO$_3$中排序为$n^*(\text{Sr})>n^*(\text{Ti})>n^*(\text{O})>n^*(\text{间隙})$，与实验Debye--Waller不确定性范围内$\sigma V_\text{col}\Delta z$的种类排序一致。
...
\caption{\textbf{诊断量-势关联图。}SrTiO$_3$ [001]在\SI{30}{\keV}下的五个共定位场：投影势、所需Taylor阶数$n^*$、发散比$r$、停滞指示器$s$和BSC反射率。在原子柱上Pearson $r(V,n^*)>0.9$。}\label{fig:correlation}
\end{figure}
...
我们在3种材料$\times$3种电压$\times$4种$\Delta z$值（$\Delta z\le\SI{1.0}{\angstrom}$，36个有效点；$\Delta z>\SI{1.0}{\angstrom}$因投影近似失效被排除）上表征了收敛性。在$t\approx\SI{200}{\angstrom}$处，出现了由原子序数$Z$主导而非二维$(\rho,\eta)$相结构主导的清晰材料排序（表\ref{tab:phase}，图\ref{fig:phase}）。Si（$Z=14$）始终收敛，SrTiO$_3$（有效$Z\approx38$）始终条件收敛（与收敛参考偏差1--5\%），Au（$Z=79$）始终发散（偏差$>10\%$）。

\begin{table}[H]
\centering
\caption{$t\approx\SI{200}{\angstrom}$处的Z主导收敛。全部36个点按材料分类。$\rho$和$\eta$范围涵盖所有电压和$\Delta z\le\SI{1.0}{\angstrom}$。尽管覆盖了$(\rho,\eta)$平面，收敛性仅由材料$Z$决定；在此厚度下$\eta$范围过窄，无法跨越相区边界。}\label{tab:phase}
\small
\begin{tabular}{lcccccc}
\toprule
材料 & $Z$ & $\rho$范围 & $\eta$范围 & 状态 & 点数 \\
\midrule
Si & 14 & 0.009--0.106 & 0.003--0.012 & 收敛 & 12 \\
SrTiO$_3$ & 混合 & 0.015--0.176 & 0.004--0.017 & 条件收敛 & 12 \\
Au & 79 & 0.031--0.372 & 0.004--0.016 & 发散 & 12 \\
\bottomrule
\end{tabular}
\end{table}
...
对SrTiO$_3$ [001]在\SI{30}{\keV}、总厚度$t=\SI{781}{\angstrom}$（200个原胞）下，扫描$\Delta z\in\{0.2,0.4,0.8,1.0\}$~\AA，通过柱上$|\psi|^2$的自相关提取Pendell\"osung周期$\xi$。结果列于表\ref{tab:channeling}。

\begin{table}[H]
\centering
\caption{SrTiO$_3$ [001]在\SI{30}{\keV}下的Pendell\"osung周期$\xi$随$\Delta z$变化。CVDMS与Fourier多层法在$\Delta z\le\SI{0.4}{\angstrom}$（$\Delta\xi=0$）处精确一致；在粗切片下出现分歧。ACF振幅反映通道效应相干性。}\label{tab:channeling}
\small
\begin{tabular}{cccccc}
\toprule
$\Delta z$ (\AA) & 切片数 & CVDMS $\xi$ (\AA) & Fourier $\xi$ (\AA) & $\Delta\xi$ (\AA) & CVDMS ACF \\
\midrule
...
对Si [001]在\SI{100}{\keV}和Au [001]在\SI{300}{\keV}重复了相同的$\Delta z$扫描（表\ref{tab:cross_mat}）。对易子信号按$\propto\sigma V\propto Z/v$标度，产生清晰的Z排序：Si（$Z=14$）在所有$\Delta z$处显示零$\Delta\xi$，而Au（$Z=79$）在所有$\Delta z$处ACF比（CVDMS/Fourier）为1.36--1.43，表明对于重元素，即使细切片下对易子也从不消失。

\begin{table}[H]
\centering
\caption{跨材料通道效应总结。$\Delta\xi_\text{max}$是$\Delta z\in\{0.4,0.8,1.0\}$~\AA{}范围内的最大周期差异。ACF比 = CVDMS ACF / Fourier ACF。}\label{tab:cross_mat}
\small
\begin{tabular}{lcccccc}
\toprule
材料 & $Z$ & $\xi_\text{bulk}$ (\AA) & $\xi_\text{meas}$ (\AA) & ACF$_C$范围 & ACF$_F$范围 & ACF比 \\
\midrule
Si & 14 & 143 & 21.6--22.0 & 0.70--0.72 & 0.70--0.72 & 1.00 \\
SrTiO$_3$ & 混合 & 65 & 51.2--54.4 & 0.34--0.42 & 0.28--0.29 & 1.23--1.53 \\
...
对SrTiO$_3$ [001]在\SI{30}{\keV}、$\Delta z=\SI{0.4}{\angstrom}$下，将探针置于Sr柱上，厚度从$t\approx\SI{20}{\angstrom}$扫描到$\SI{625}{\angstrom}$（表\ref{tab:thickness}）。结果揭示了对易子误差的近线性累积：$1-\text{NCC}\propto t^{1.22}$，相位RMS $\propto t^{1.02}$，而衍射图案NCC全程保持$>0.97$（图\ref{fig:thickness_sweep}），表明衍射观测量比实空间波函数对对易子误差更鲁棒。定量分析的操作包络为：NCC $>0.99$（$t<\SI{80}{\angstrom}$），NCC $>0.95$（$t<\SI{200}{\angstrom}$）；对于更厚的样品，必须细化$\Delta z$或接受已知的对易子偏差。

\begin{table}[H]
\centering
\caption{厚度扫描：SrTiO$_3$ [001]在\SI{30}{\keV}、$\Delta z=\SI{0.4}{\angstrom}$，探针在Sr柱上。NCC和相位RMS相对于$\Delta z\to0$ Fourier参考。}\label{tab:thickness}
\small
\begin{tabular}{cccccc}
\toprule
$t$ (\AA) & 切片数 & NCC & $1-$NCC & 相位RMS (mrad) & DP NCC \\
\midrule
...
在细采样（\SI{0.05}{\angstrom\per\pixel}）下，抗混叠是强制性的：禁用抗混叠时，模拟立即溢出（所有指标为NaN/Inf）。启用抗混叠时，NCC = 0.987相对于更细$\Delta z=\SI{0.2}{\angstrom}$参考，高频功率比为$\sim10^{-15}$——确认了完全的带宽控制（表\ref{tab:antialias}）。剩余的NCC间隙$\sim$0.013反映了$\Delta z$加倍产生的真实对易子误差，与第\ref{sec:convergence}节建立的$\mathcal O(\Delta z^2)$标度一致。

\begin{table}[H]
\centering
\caption{抗混叠对比：SrTiO$_3$ [001]在\SI{30}{\keV}、\SI{0.05}{\angstrom\per\pixel}、$\Delta z=\SI{0.4}{\angstrom}$相对于$\Delta z=\SI{0.2}{\angstrom}$参考。}\label{tab:antialias}
\small
\begin{tabular}{lcc}
\toprule
指标 & AA = 开 & AA = 关 \\
\midrule
NCC vs.\ 参考 & 0.98674 & 溢出 \\
相位RMS (mrad) & 437 & 溢出 \\
振幅RMS误差 & 0.155 & 溢出 \\
高频功率比 & $\sim10^{-15}$ & 溢出 \\
\bottomrule
...
\caption{\textbf{细采样下抗混叠的必要性。}在\SI{0.05}{\angstrom\per\pixel}下AA=开与AA=关的对比。AA=关立即溢出；AA=开维持稳定传播，NCC=0.987相对于更细$\Delta z$参考。}\label{fig:antialias}
\end{figure}
...
\subsection{通道效应、HOLZ与被丢弃对易子的物理含义}

Lie--Trotter分裂$\exp(i\Delta z\hat A)\exp(i\Delta z\hat B)$以每片$-\tfrac12\Delta z^2[\hat A,\hat B]+\mathcal O(\Delta z^3)$的代价替代$\exp(i\Delta z(\hat A+\hat B))$，全局累积为$\mathcal O(\Delta z)$。这可从Baker--Campbell--Hausdorff（BCH）公式直接看出
...
\subsection{与Lie--Trotter分裂和Bloch波方法的关系}

传统Fourier多层法~\cite{Kirkland2020,CowleyMoodie1957,MadsenSusi2021}精确守恒$\|\psi\|$，并在abTEM~\cite{MadsenSusi2021}、PRISM~\cite{Ophus2017}和MULTEM~\cite{LobatoVanDyck2015}中得到了充分验证；其对易子误差通过减小$\Delta z$直到结果停止变化来控制——这是一种\textbf{间接的}、全局的收敛测试，需要多次运行且无法定位误差来源。Bloch波方法~\cite{Kirkland2020}通过保留的束数控制精度，为轴向通道效应提供了自然的参考，但要求周期性势且在HRTEM尺度的束数下计算代价高昂。

CVDMS可追溯到Coene和Van Dyck~\cite{CoeneVanDyck1984}的实空间多层法算法，该算法在二维网格上使用有限差分Laplacian。Wacker和Schr\"oder~\cite{WackerSchroder2015}系统比较了实空间和Fourier空间多层法算法，讨论了Laplacian离散化精度——这一问题被本工作的FD/FFT双后端设计直接继承（第\ref{sec:laplacian}节）。CVDMS通过为同一算符$\hat{K}$提供FD和FFT两种Laplacian后端，桥接了这两个传统。

CVDMS在现有方法中占据第三位置：它保留了多层法的几何灵活性，同时\textbf{原位暴露}了原本隐藏的对易子误差作为逐像素量，使收敛性成为样品在每个点上的属性，而非整个模拟的属性。因此我们将其作为一种\textbf{不同的受控近似}，而非分裂方法或Bloch波方法的替代品。

\subsection{对定量TEM模拟的意义}

对于普通TEM从业者，本研究的可操作输出有三个方面。第一，生产默认值$\Delta z=\SI{0.4}{\angstrom}$、$\varepsilon=10^{-7}$现在得到了定量验证而非启发式采纳：它在全部三类晶体中提供NCC $>1-10^{-5}$，且安全地位于双侧最优窗口分析所识别的对易子-发散阈值下方（对SrTiO$_3$在\SI{30}{\keV}下为0.6--0.7~\AA{}，第\ref{sec:float32}节）。第二，逐像素诊断量使得\textbf{单次$\Delta z$选择}成为可能：用户可运行一次模拟并检查$n^*(\mathbf{R})$和$r(\mathbf{R})$来识别需要局域$\Delta z$细化的区域，取代传统的多次$\Delta z$扫描收敛测试。第三，$t=\SI{200}{\angstrom}$处的Z主导收敛（第\ref{sec:operating-envelope}节）提供了简单的材料依赖指南：轻元素（$Z\lesssim14$）在标准参数下无条件收敛；中等元素（$Z\sim38$）需要监测停滞诊断量；重元素（$Z\gtrsim79$）无论其他参数如何选择都需要减小$\Delta z$或使用float64精度。

\begin{enumerate}
\item \textbf{样品感知的决策准则。}$(\rho,\eta)$参数化（第\ref{sec:operating-envelope}节）提供了量纲正确的收敛阈值，而非全局$\Delta z$经验规则：同一网格上的SrTiO$_3$薄膜和Au团簇接受不同的有效$\Delta z$容限，区域边界由仅从结构因子$V_g$可估算的长度比设定。
\item \textbf{自适应切片。}$V(\mathbf{R})\leftrightarrow n^*(\mathbf{R})$关联（第\ref{sec:diagnostic-potential}节）为仅在诊断量标记处细化$\Delta z$（而非全局细化）开辟了道路——通过单次CVDMS运行即可实现，该运行同时产生出射波和$n^*(\mathbf{R})$图，标识哪些像素需要更细切片。
\item \textbf{异质结构界面建模。}Fresnel BSC残差是异质结构建模中可直接测量的量，扩展到真空/样品和衬底/样品边界——简化背散射近似在这些界面可产生$\sim$17\%的通量误差，传播到下游CBED盘中。
\item \textbf{标准参数下的通道效应和HOLZ。}对于目前使用Bloch波参考~\cite{Kirkland2020,PennycookNellist2011,Allen2003}的定量衍射分析，第\ref{sec:channeling-holz}节的通道效应和HOLZ可观测量确立了CVDMS与Fourier多层法在标准生产$\Delta z\le\SI{0.4}{\angstrom}$下不可区分，而可测量的对易子驱动分歧仅在$\Delta z\ge\SI{0.8}{\angstrom}$出现——表明常规切片厚度下的常规多层法已经捕获了相关的通道效应物理，逐像素诊断量提供了一个\textit{先验}指标，指示何时需要更细切片。
\item \textbf{Float32精度底噪。}对于重元素低电压场景（如Au在\SI{30}{\keV}下），float32抵消构成任何算法的精度底噪——而非CVDMS的局限，应予以报告；建议使用float64 K级数作为补救措施。
\end{enumerate}

\subsection{局限性与展望}

\begin{enumerate}
\item \textbf{仅弹性散射。}非弹性过程（等离激元、声子~\cite{Forbes2010}）需要单独处理；CVDMS提供了弹性基础，第\ref{sec:conserved-observables}节的IPR诊断量是与声子通道耦合的自然起点。
\item \textbf{投影势前提。}诊断量指示投影近似何时失效，但不修复它。在发散区域（$\rho\gg1$），需要全三维方法~\cite{LobatoVanDyck2014}。
\item \textbf{计算代价。}在同等收敛设置下，每片代价超过Lie--Trotter多层法——这是内K级数和逐像素诊断量的代价。因此CVDMS的产出不是在固定$\Delta z$下更快的墙上时间，而是能够\textbf{认证}样品上的局域收敛性，并在模拟完成前识别Lie--Trotter结果将默默继承被忽略对易子的区域。
\item \textbf{验证包络。}定量论断基于三类晶体（轻共价键Si、混合离子SrTiO$_3$、重金属Au）和三种能量（30/100/300~keV）。外推到二维材料、非晶样品或低于\SI{30}{\keV}的束流需要重新运行Phase~0.5协议；诊断量本身不保证验证包络外的精度。
\item \textbf{未来工作。}由实时$n^*(\mathbf{R})$驱动的自适应$\Delta z$；混合精度K级数；与声子和叠层衍射4D-STEM工作流~\cite{Savitzky2021}的耦合；扩展到背散射主导的RHEED。
\end{enumerate}

\subsection{守恒可观测量}\label{sec:conserved-observables}

\textbf{横向连续性。}连续性方程$\partial_z|\psi|^2+\nabla_\perp\cdot\mathbf{j}_\perp=0$的逐像素验证表明，偏差$>10^{-4}$的像素落在发散区域，且与停滞图位置精确重合（第\ref{sec:diagnostic-potential}节）。

\textbf{逆参与比。}$\text{IPR}(z)=\sum|\psi|^4/(\sum|\psi|^2)^2$追踪原子柱上束流局域化的程度。对SrTiO$_3$在\SI{30}{\keV}下，有和无BSC的CVDMS产生几乎相同的IPR($t$)（平均IPR$\times$面积$\approx11$），确认背散射在此能量下不扰动通道效应态。

\textbf{CBED盘强度$I_g(t)$。}$\{200\}$、$\{220\}$、$\{420\}$盘强度随厚度变化（在对称孔径上积分）被报告为主要的\textbf{实验可测量观测量}。CVDMS相对于$\Delta z\to0$参考的偏差在所有三个盘中$t\le\SI{400}{\angstrom}$时均$<2\%$。
...
\begin{table}[H]
\centering
\caption{Laplacian后端实现。}\label{tab:laplacian}
\small
\begin{tabular}{lll}
\toprule
路径 & 实现 & 细节 \\
\midrule
FD精度2 & 3点可分离1D模板 & 标准5点2D Laplacian \\
FD精度4 & 5点可分离1D模板 & 半径2 \\
FD精度6 & 7点可分离1D模板 & 周期边界 \\
FD精度8 & 9点可分离1D模板 & 默认；非3$\times$3紧致模板 \\
FFT & cuFFT C2C & $-4\pi^2|\mathbf{k}|^2$乘法 \\
\bottomrule
\end{tabular}
\end{table}
