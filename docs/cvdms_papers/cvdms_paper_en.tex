...
\textbf{Spatial structure.} The projected potential $V(\mathbf{R})=\sum_a V_a(\mathbf{R}-\mathbf{R}_a)$ peaks at atomic column positions, with $V_\text{peak}\propto Z^{4/3}$ and $1/e$ width $w_\text{col}\sim\SIrange{0.4}{0.7}{\angstrom}$, dropping 1--3 orders of magnitude in interstitial regions~\cite{LobatoVanDyck2014}. Using the Lobato parameterization~\cite{LobatoVanDyck2014} with isotropic Debye--Waller factors (Sr: 0.62, Ti: 0.51, O: 0.86, Si: 0.46, Au: 0.66~\SI{}{\angstrom\squared} at \SI{300}{\kelvin}~\cite{Forbes2010}), representative DW-broadened peak values are: SrTiO$_3$~[001] Sr column $V_\text{peak}\approx\SI{1000}{\electronvolt\angstrom}$; Si~[001] column $\approx\SI{500}{\electronvolt\angstrom}$; Au~[001] column $\approx\SI{1600}{\electronvolt\angstrom}$ (static: \SI{3120}{\electronvolt\angstrom}, \SI{1270}{\electronvolt\angstrom}, \SI{5870}{\electronvolt\angstrom} respectively; DW broadening reduces peak values by 61--73\%; Supplementary Data~P1-DW).
...
The spatial correlation between the potential and the diagnostics is quantitative. Since the outer Taylor term $w_n\propto(\sigma V\Delta z)^n/n!$ grows with the local potential strength, the required order $n^*(\mathbf{R})$ is expected to be monotonic in $V(\mathbf{R})$. This is confirmed by the strong Pearson correlation between the projected potential and its gradient magnitude, $r(V,|\nabla V|)=0.96$ (Supplementary Data~P7), indicating that commutator-sensitive regions---where $V$ is large and $\nabla V$ is steep---coincide with atomic column positions. Within one SrTiO$_3$ unit cell the projected potential orders as $V(\text{Sr})=\SI{3120}{\electronvolt\angstrom}>V(\text{Ti-O})=\SI{2700}{\electronvolt\angstrom}>V(\text{O})=\SI{770}{\electronvolt\angstrom}>V(\text{interstitial})=\SI{37}{\electronvolt\angstrom}$ (static values, \SI{0.02}{\angstrom\per\pixel} sampling), so the expected $n^*$ ordering is $n^*(\text{Sr})>n^*(\text{Ti-O})>n^*(\text{O})>n^*(\text{interstitial})$. Fresnel BSC reflectance $|R(\mathbf{R})|^2$ correlates with $|\partial_z V|^2$, as predicted by the first-order continuity expansion (Section~\ref{sec:bsc}).
...
\caption{\textbf{Diagnostic-potential correlation maps.} Five co-localized fields for SrTiO$_3$ [001] at \SI{30}{\keV}: projected potential, required Taylor order $n^*$, divergence ratio $r$, stagnation indicator $s$, and BSC reflectance. The $n^*$ distribution follows $V(\mathbf{R})$, with species ordering $n^*(\text{Sr})>n^*(\text{Ti-O})>n^*(\text{O})>n^*(\text{interstitial})$ matching $\sigma V_\text{col}\Delta z$.}\label{fig:correlation}
\end{figure}
...
For SrTiO$_3$ [001] at \SI{30}{\keV}, total thickness $t=\SI{781}{\angstrom}$ (200 unit cells), sweeping $\Delta z\in\{0.2,0.4,0.8,1.0\}$~\AA, the Pendell\"osung period $\xi$ was extracted via autocorrelation of on-column $|\psi|^2$. Results are listed in Supplementary Table~S2.
...
The same $\Delta z$ sweep was repeated for Si [001] at \SI{100}{\keV} and Au [001] at \SI{300}{\keV} (Supplementary Table~S3). The commutator signal scales as $\propto\sigma V\propto Z/v$, producing a clear Z-ordering: Si ($Z=14$) shows zero $\Delta\xi$ at all $\Delta z$, while Au ($Z=79$) exhibits an ACF ratio (CVDMS/Fourier) of 1.36--1.43 at all $\Delta z$, indicating that for heavy elements the commutator is never negligible even at fine slicing.
...
For SrTiO$_3$ [001] at \SI{30}{\keV} with $\Delta z=\SI{0.4}{\angstrom}$, the probe was placed on the Sr column and thickness was swept from $t\approx\SI{20}{\angstrom}$ to $\SI{625}{\angstrom}$ (Supplementary Table~S4). The results reveal a near-linear accumulation of commutator error: $1-\text{NCC}\propto t^{1.22}$ and phase RMS $\propto t^{1.02}$, while the diffraction pattern NCC remains $\ge0.97$ throughout (Fig.~\ref{fig:thickness_sweep}), indicating that diffraction observables are more robust to commutator error than real-space wavefunctions. The operational envelope for quantitative analysis places NCC $>0.99$ for $t<\SI{80}{\angstrom}$ and NCC $>0.95$ for $t<\SI{200}{\angstrom}$; for thicker specimens, $\Delta z$ refinement or acceptance of known commutator bias becomes necessary.
...
The physical origin of this behavior lies in the commutator discarded by conventional splitting methods. The Lie--Trotter splitting $\exp(i\Delta z\hat A)\exp(i\Delta z\hat B)$ replaces $\exp(i\Delta z(\hat A+\hat B))$ at a cost of $-\tfrac12\Delta z^2[\hat A,\hat B]+\mathcal O(\Delta z^3)$ per slice, accumulating globally to $\mathcal O(\Delta z)$. This can be seen directly from the Baker--Campbell--Hausdorff (BCH) formula
...
Turning to the relationship with existing methods, conventional Fourier multislice~\cite{Kirkland2020,CowleyMoodie1957,MadsenSusi2021} exactly conserves $\|\psi\|$ and is well-validated in abTEM~\cite{MadsenSusi2021}, prismatic~\cite{Ophus2017}, and MULTEM~\cite{LobatoVanDyck2015}; its commutator error is controlled by reducing $\Delta z$ until results stop changing---an \textbf{indirect}, global convergence test requiring multiple runs and unable to localize error sources. Bloch-wave methods~\cite{Kirkland2020} control accuracy through the number of retained beams and provide a natural reference for axial channeling, but require periodic potentials and become computationally expensive at HRTEM-scale beam counts. CVDMS traces back to the real-space multislice algorithm of Coene \& Van Dyck~\cite{CoeneVanDyck1984}, which used finite-difference Laplacians on 2D grids. Wacker \& Schr\"oder~\cite{WackerSchroder2015} systematically compared real-space and Fourier-space multislice algorithms, discussing Laplacian discretization accuracy---a concern directly inherited by the present FD/FFT dual-backend design (Section~\ref{sec:laplacian}). CVDMS bridges these two traditions by offering both FD and FFT Laplacian backends for the same operator $\hat{K}$. It thus occupies a third position among existing methods: it retains the geometric flexibility of multislice while \textbf{exposing in situ} the otherwise-hidden commutator error as per-pixel quantities, making convergence a property of the specimen at each point rather than of the simulation as a whole. We therefore present it as a \textbf{different controlled approximation}, not a replacement for splitting or Bloch-wave methods.

For the average TEM practitioner, the actionable output of this study is threefold. First, the production default $\Delta z=\SI{0.4}{\angstrom}$, $\varepsilon=10^{-7}$ is now quantitatively validated rather than heuristically adopted: it delivers NCC $>1-10^{-5}$ across all three crystal classes and lies safely below the commutator-divergence threshold identified by the two-sided optimal-window analysis (0.6--0.7~\AA\ for SrTiO$_3$ at \SI{30}{\keV}, Section~\ref{sec:float32}). Second, the per-pixel diagnostics enable \textbf{single-pass $\Delta z$ selection}: a user can run one simulation and inspect $n^*(\mathbf{R})$ and $r(\mathbf{R})$ to identify regions requiring local $\Delta z$ refinement, replacing the traditional multi-run $\Delta z$-sweep convergence test. Third, the Z-dominated convergence at $t=\SI{200}{\angstrom}$ (Section~\ref{sec:operating-envelope}) provides a simple material-dependent guideline: light elements ($Z\lesssim14$) are unconditionally convergent at standard parameters; intermediate elements ($Z\sim38$) require monitoring of stagnation diagnostics; heavy elements ($Z\gtrsim79$) demand reduced $\Delta z$ or float64 precision regardless of other parameter choices. Beyond these immediate takeaways, the $(\rho,\eta)$ parameterization (Section~\ref{sec:operating-envelope}) provides dimensionally correct convergence thresholds rather than global $\Delta z$ rules of thumb: an SrTiO$_3$ film and an Au cluster on the same grid accept different effective $\Delta z$ tolerances, with regional boundaries set by length ratios estimable from structure factors $V_g$ alone. The $V(\mathbf{R})\leftrightarrow n^*(\mathbf{R})$ correlation (Section~\ref{sec:diagnostic-potential}) opens the path to adaptive slicing, where $\Delta z$ is refined only where diagnostics flag it, rather than globally---enabled by a single CVDMS run that produces both the exit wave and the $n^*(\mathbf{R})$ map identifying which pixels need finer slicing. The Fresnel BSC residual provides a directly measurable quantity for heterostructure interface modeling, extending to vacuum/specimen and substrate/specimen boundaries, where the simplified backscattering approximation can produce $\sim$17\% flux error propagating into downstream CBED disks. For quantitative diffraction analysis currently using Bloch-wave references~\cite{Kirkland2020,PennycookNellist2011,Allen2003}, the channeling and HOLZ observables of Section~\ref{sec:channeling-holz} establish that CVDMS and Fourier multislice are indistinguishable at standard production $\Delta z\le\SI{0.4}{\angstrom}$, while measurable commutator-driven divergence appears only at $\Delta z\ge\SI{0.8}{\angstrom}$---indicating that routine multislice at conventional slice thickness already captures the relevant channeling physics, with per-pixel diagnostics providing an \textit{a priori} indicator of when finer slicing is needed. Finally, for heavy-element low-voltage scenarios (e.g.\ Au at \SI{30}{\keV}), float32 cancellation constitutes a precision floor for any algorithm---not a limitation of CVDMS---and float64 K-series is recommended as the remedy.

Several limitations define the current scope of this study. Inelastic processes (plasmons, phonons~\cite{Forbes2010}) require separate treatment; CVDMS provides the elastic foundation, and the IPR diagnostic discussed below is a natural starting point for coupling to phonon channels. The projected-potential premise remains: the diagnostics indicate when the projected approximation fails but do not repair it; in the divergent regime ($\rho\gg1$), a fully three-dimensional method is required~\cite{LobatoVanDyck2014}. At equal convergence settings, the per-slice cost exceeds that of Lie--Trotter multislice---the price of the inner K-series and per-pixel diagnostics; the deliverable of CVDMS is therefore not faster wall-clock time at fixed $\Delta z$, but the ability to \textbf{certify} local convergence on the specimen and identify, before the simulation completes, the regions where a Lie--Trotter result would silently inherit the omitted commutator. The quantitative claims are anchored to three crystal classes (light covalent Si, mixed ionic SrTiO$_3$, heavy metal Au) and three energies (30/100/300~keV); extrapolation to 2D materials, amorphous specimens, or beams below \SI{30}{\keV} requires re-running the Phase~0.5 protocol, and the diagnostics themselves do not guarantee accuracy outside this validation envelope. Future work includes adaptive $\Delta z$ driven by real-time $n^*(\mathbf{R})$, mixed-precision K-series, coupling to phonon and ptychographic 4D-STEM workflows~\cite{Savitzky2021}, and extension to backscattering-dominated RHEED.

Three conserved observables serve as independent cross-checks on the CVDMS framework.\label{sec:conserved-observables} The inverse participation ratio IPR($z$)$=\sum|\psi|^4/(\sum|\psi|^2)^2$ tracks the degree of beam localization on atomic columns; for SrTiO$_3$ at \SI{30}{\keV}, CVDMS with BSC yields mean IPR$\times$area $=11.2$ and CVDMS without BSC yields $11.2$, confirming that backscattering does not perturb channeling states at this energy. The per-pixel continuity residual $|\partial_z|\psi|^2+\nabla_\perp\cdot\mathbf{j}_\perp|$ correlates with the local potential $V(\mathbf{R})$ (Pearson $r=0.74$ between $V$ and the per-pixel maximum residual, Supplementary Data~P6), consistent with the interpretation that commutator error is largest where the potential gradient is steepest. For the experimentally accessible $\{200\}$, $\{220\}$, and $\{420\}$ CBED disk intensities integrated over symmetric apertures (Supplementary Data~P5): at $t\le\SI{200}{\angstrom}$, CVDMS deviation from the $\Delta z\to0$ Fourier reference is $<6\%$ for all three disks; the $\{200\}$ disk, the strongest and most experimentally relevant, deviates by $<4\%$ at $t\le\SI{400}{\angstrom}$; higher-order disks $\{220\}$ and $\{420\}$ show increasing commutator sensitivity at larger thicknesses (16\% and 36\% at $t\approx\SI{390}{\angstrom}$), consistent with their larger scattering vectors sampling finer spatial frequencies where the projected-potential approximation degrades.
